...
<tr class="{{ $usuario->activo === 'n' ? 'text-red-600' : '' }}">
    ...
    <td>
        <nobr>
            <button class="btn btn-xs btn-default text-primary mx-1 shadow edit-button" title="Edit" data-toggle="modal" data-target="#modalPurple"   data-usuario-activo="{{ $usuario->activo }}"   data-id="{{ $usuario->id }}" data-nombre="{{ $usuario->name }}" data-rol-id="{{ $usuario->roles->pluck('id')->implode(',') }}" data-clientes-seleccionados="{{ $usuario->clientes->pluck('id')->implode(',') }}">
                <i class="fa fa-lg fa-fw fa-pen"></i>
            </button>
            <form id="deshabilitar-form-{{ $usuario->id }}" action="{{ route('usuarios.destroy', $usuario->id) }}" method="POST" class="inline delete-form">
                @csrf
                @method('DELETE')
                <button type="button" onclick="confirmDeshabilitar('{{ $usuario->id }}')" class="btn btn-xs btn-default text-danger mx-1 shadow" title="Delete">
